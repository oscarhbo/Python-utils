...
def cargar_tablas_respaldo():
    ruta = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Tablas_alianza.txt")
    ...
    def descargar_alianza(self):
        ...
        try:
            ...
            for tabla in tablas_orden:
                ...
                inserts.append("PROMPT ------------------------------------------------------------------------------------------------\n")
                cursor.execute(f"SELECT * FROM GORAPR.{tabla} WHERE codalianza = :1", [codalianza])
            ...
            if carpeta:
                ...
                with open(ruta_ambi, "w", encoding="utf-8") as f:
                    f.write("\n".join(inserts))
