...
fechaBase = (datetime.now() - timedelta(days=1)).replace(hour=4, minute=0, second=0, microsecond=0)
#fechaBase = datetime(year=2024, month=5, day=10, hour=14, minute=30, second=45, tzinfo=timezone.utc)

print(f"La fecha base para el incremental es: {fechaBase}")
...
with Session() as session:
    try:
        ...
        query = text("SELECT * FROM ORDERS WHERE LastUpdated > :last_updated")

        result = session.execute(query, {'last_updated': fechaBase}).fetchall()
